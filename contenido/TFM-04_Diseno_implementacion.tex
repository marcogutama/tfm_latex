...
\begin{itemize}
    ...
    \item \textbf{Etapa 'Static Analysis'}: Para optimizar el tiempo (cumpliendo \textbf{RNF-01}), esta etapa utiliza un bloque \texttt{parallel} para ejecutar dos tareas simultáneamente. Mientras una rama ejecuta el análisis SAST con SpotBugs, la otra recopila la lista de archivos Java. Esta paralelización asegura que la lista de ficheros a analizar por la IA esté lista tan pronto como sea posible, permitiendo que la etapa de `AI-Powered Analysis` pueda comenzar sin demoras innecesarias. El uso de \texttt{try-catch} dentro de la rama de SpotBugs añade resiliencia: un fallo en esta herramienta no detendrá el pipeline, simplemente se registrará.
