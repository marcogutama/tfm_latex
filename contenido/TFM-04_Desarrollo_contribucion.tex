...
Los requisitos funcionales describen lo que el sistema debe \textit{hacer}. Se definieron los siguientes:
...
\subsubsection{RF-04: Procesamiento de Resultados de Escáneres}

\textbf{Descripción:} El sistema debe ser capaz de leer y entender los reportes generados por las herramientas SAST y SCA.

\textbf{Justificación:} Los reportes en bruto (XML, JSON) no son útiles por sí solos. El sistema necesita extraer la información clave (tipo de vulnerabilidad, archivo afectado, línea de código) para poder actuar sobre ella.

\textbf{Criterio de Aceptación:} Un script dentro del pipeline puede parsear los archivos `dependency-check-report.json` y `spotbugsXml.xml` para crear una lista estructurada de vulnerabilidades.

\subsubsection{RF-05: Generación de Sugerencias de Mitigación con IA}

\textbf{Descripción:} Para cada vulnerabilidad de código fuente encontrada, el sistema debe enviar el fragmento de código relevante a un modelo de IA y solicitar una sugerencia de corrección.
...
\subsubsection{RF-06: Generación de Reportes Consolidados y Accionables}

\textbf{Descripción:} El sistema debe crear un único reporte en formato HTML que combine los resultados de todos los análisis (SCA, SAST y IA) de una manera fácil de entender.

\textbf{Justificación:} Los desarrolladores no deberían tener que revisar tres reportes diferentes. Un único panel de control con toda la información relevante mejora drásticamente la usabilidad.
...
\subsubsection{RF-07: Implementación de ``Quality Gates''}
...
Los requisitos no funcionales describen \textit{cómo} debe ser el sistema en términos de calidad y rendimiento.
...
\textbf{Criterio de Aceptación:} El tiempo total de ejecución del pipeline en un microservicio de tamaño medio no debe aumentar en más de 5 minutos debido a la etapa de análisis con IA.
...
\begin{itemize}
    ...
    \item \textbf{Snyk / Mend (antes WhiteSource):} Son soluciones comerciales de SCA muy potentes que a menudo ofrecen una mejor precisión y menos falsos positivos que Dependency-Check. Sin embargo, para un proyecto de tesis, una herramienta de código abierto como OWASP Dependency-Check era la opción más adecuada por su accesibilidad.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Scripts de Inicialización:} Se copian los scripts `.groovy` al directorio `init.groovy.d/`. Jenkins está diseñado para ejecutar cualquier script en este directorio la primera vez que arranca con un directorio `/var/jenkins\_home` vacío. Esto permite una configuración "cero-toque" del sistema.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Todos los detalles (la URL del repositorio, la rama, la ruta al `Jenkinsfile`) se leen de variables de entorno, haciendo que la creación del pipeline sea totalmente configurable desde el exterior.
...
El archivo \texttt{Jenkinsfile} constituye el cerebro de la automatización de esta tesis. Define, a través de una sintaxis declarativa (\textit{Pipeline as Code}), todo el flujo de trabajo de Integración Continua, Análisis de Seguridad y Despliegue Continuo (CI/Sec/CD). Su propósito es orquestar de manera automática todas las herramientas y procesos, desde que el desarrollador sube el código hasta que se genera un reporte de seguridad accionable.

A continuación, se desglosa su estructura y funcionamiento.

\subsubsection{Estructura y Entorno del Pipeline}
El pipeline utiliza la sintaxis Declarativa y define un `environment` global. La variable `SCAN\_RESULTS\_DIR` se define aquí para proporcionar una ruta consistente y centralizada donde todas las etapas pueden leer o escribir artefactos de análisis.


\subsubsection{Configuración Inicial (Bloques tools y environment)}

El pipeline comienza estableciendo su entorno de ejecución \textbf{environment}, donde se definen las variables de entorno que estarán disponibles a lo largo de toda la ejecución. Es de especial importancia la línea: 
    
\texttt{OPENROUTER\_API\_KEY = credentials('openrouter-api-key-id')}, que demuestra una práctica de seguridad fundamental: la gestión de secretos. La clave de la API para el modelo de IA no está escrita en el código, sino que se obtiene del gestor de credenciales seguro de Jenkins.



\subsubsection{Flujo de Trabajo por Etapas (stages)}
Cada etapa representa una unidad lógica de trabajo, ejecutada en secuencia, a menos que se indique lo contrario.
\begin{itemize}
    \item \textbf{Etapa 'Checkout \& Build'}: Esta etapa prepara el código fuente y lo compila. El comando `mvn -Dmaven.test.failure.ignore=true clean package` es deliberado: `clean` asegura que no queden artefactos de compilaciones anteriores, `package` compila el código y lo empaqueta en un `.jar`, y `-Dmaven.test.failure.ignore=true` permite que el pipeline continúe al análisis de seguridad incluso si las pruebas unitarias fallan.

    \item \textbf{Etapa 'Unit Test Reports'}: Utiliza el paso `junit` para parsear los reportes XML generados por Maven. Jenkins utiliza estos datos para mostrar un historial de resultados de pruebas, visualizar tendencias y permitir un análisis detallado de los fallos de las pruebas unitarias.

    \item \textbf{Etapa 'Static Analysis'}: Esta etapa utiliza un bloque `parallel` para ejecutar dos tareas simultáneamente, reduciendo el tiempo total del pipeline. Mientras una hebra ejecuta el análisis con SpotBugs, la otra recopila la lista de archivos Java.

    \item \textbf{Etapa 'AI-Powered Analysis'}: Es el núcleo del proyecto. El paso `dir('scan-results')` aísla la ejecución del script y sus salidas en un directorio específico. El bloque `withCredentials` inyecta de forma segura la clave de la API, que el script de Python lee luego como una variable de entorno.

    \item \textbf{Etapa 'Quality Gate'}: Este es el control de seguridad automatizado. Su lógica implementa una política de "cero tolerancia" para vulnerabilidades críticas. El uso del paso `error()` es una decisión de diseño crucial: detiene el pipeline de forma inmediata y lo marca como `FAILURE`, previniendo físicamente que un artefacto inseguro pueda ser construido o desplegado.

    \item \textbf{Etapas 'Build Docker Image' y 'Local Deploy'}: Estas etapas, que solo se ejecutan si el Quality Gate es superado, completan el ciclo de Despliegue Continuo. El comando `docker build -f demo/Dockerfile -t demo-app .` se desglosa en: `-f` para especificar la ruta al Dockerfile, `-t` para etiquetar la imagen, y `.` para definir el contexto de construcción.
\end{itemize}

\subsubsection{Acciones Finales y Reportes (post)}

El bloque \texttt{post} define acciones que se ejecutan al finalizar el pipeline, independientemente del resultado:
\begin{itemize}
    \item \textbf{always}: Se ejecuta siempre. Es responsable de publicar los resultados de las pruebas unitarias (\texttt{junit}), archivar todos los artefactos generados para trazabilidad (\texttt{archiveArtifacts}) y, lo más importante, publicar el reporte HTML del análisis de IA (\texttt{publishHTML}). Esto hace que el reporte sea fácilmente accesible desde la interfaz de Jenkins.
    \item \textbf{success, unstable, failure}: Permiten enviar notificaciones o ejecutar acciones específicas dependiendo del estado final del pipeline, proporcionando una retroalimentación clara y concisa.
\end{itemize}

Para mantener la claridad y brevedad en la descripción del sistema, el contenido completo del archivo más extenso se ha documentado en el \textbf{Anexo \ref{anexo:jenkinsfile}}.


\subsubsection{El Script de IA: Componente Integrador del Sistema}
\label{sec:ai_script}

El corazón de la innovación de este proyecto reside en el script de Python (\texttt{ai\_analyzer.py}) que se ejecuta en la etapa AI-Powered Analysis. Este script actúa como el ``componente de integración'' que une el mundo del análisis de seguridad tradicional con el poder de los modelos de lenguaje.

El script realiza las siguientes tareas:
\begin{enumerate}
    \item \textbf{Recopila el Contexto:} Primero, parsea los resultados de SpotBugs y OWASP Dependency-Check. Aunque en esta versión del prototipo el foco principal está en analizar el código fuente basado en los hallazgos de SpotBugs, tener los datos de las dependencias disponibles permite futuras mejoras.
    \item \textbf{Identifica los Ficheros a Analizar:} Busca todos los archivos `.java` en el código fuente del proyecto.
    \item \textbf{Itera y Consulta a la IA:} Para cada archivo Java, construye un \textit{prompt} cuidadosamente diseñado y lo envía a la API de OpenRouter.
    \item \textbf{Procesa la Respuesta de la IA:} Recibe la respuesta en formato JSON de la IA y la almacena.
    \item \textbf{Genera los Reportes Finales:} Utiliza todos los datos recopilados para generar dos artefactos de salida:
        \begin{itemize}
            \item `analysis-results.json`: Un archivo JSON crudo con todos los datos, ideal para el procesamiento automático por parte de los Quality Gates.
            \item `ai-analysis-report.html`: Un reporte HTML visualmente atractivo y fácil de leer para los desarrolladores.
        \end{itemize}
\end{enumerate}

El \textbf{Anexo \ref{anexo:ai_analyzer}} muestra el código completo del script, ya que es una pieza central de la implementación.

\subsubsection{El Arte de la Ingeniería de Prompts}

La calidad de los resultados de un LLM es directamente proporcional a la calidad del prompt. El prompt utilizado en ``analyze\_code\_security'' (mostrado en la sección anterior) no fue la primera versión. Se llegó a él a través de un proceso iterativo de refinamiento, basado en los siguientes principios:

\begin{itemize}
    \item \textbf{Asignación de Rol y Tarea (Role-playing):} La primera línea, ``Actúa como un experto analista de seguridad de aplicaciones Java y un revisor de código senior'', es crucial. Establece un contexto para el modelo, indicándole qué ``personalidad'' y base de conocimientos debe adoptar. Esto produce resultados significativamente mejores que una pregunta genérica.
    \item \textbf{Instrucciones Específicas:} En lugar de decir ``busca vulnerabilidades'', se proporciona una lista numerada de tipos de fallos de seguridad. Esto actúa como una checklist para el modelo, forzándolo a considerar diferentes vectores de ataque y reduciendo la probabilidad de que omita algo obvio.
    \item \textbf{Contextualización con el Código:} Al incluir el código fuente completo dentro del prompt con la etiqueta ``java'', el modelo puede analizar las interacciones entre diferentes partes del código, en lugar de analizar una línea de forma aislada.
    \item \textbf{Formato de Salida Estricto:} Esta es quizás la parte más importante para la automatización. La instrucción ``RESPONDE ÚNICA Y EXCLUSIVAMENTE CON UN OBJETO JSON VÁLIDO. No incluyas texto antes o después del JSON.'' es una directiva fuerte. Junto con el ejemplo de la estructura JSON deseada, aumenta drásticamente la probabilidad de que la salida del modelo pueda ser parseada programáticamente sin errores. Sin esta instrucción, los modelos a menudo añaden texto conversacional como ``Claro, aquí tienes el JSON con los resultados:'', lo que rompería el pipeline.
    \item \textbf{Petición de Ejemplos de Código:} Al solicitar explícitamente ``Incluye un ejemplo de código corregido si es posible'', se le pide al modelo que vaya un paso más allá de la descripción y ofrezca una solución tangible, lo que es inmensamente valioso para el desarrollador.
\end{itemize}
...
\textbf{Problema:} El uso de la API de OpenRouter.ai requiere una clave de API, que es un secreto que no debe ser expuesto en el código fuente (en el `Jenkinsfile`).
...
A continuación, se detallan las vulnerabilidades presentes en el código de prueba:
...
Para llevar a cabo una evaluación exhaustiva y comparativa, se seleccionó un conjunto de modelos de Inteligencia Artificial de vanguardia, todos accesibles a través de la plataforma OpenRouter.ai, junto con una herramienta de análisis estático de código que sirve como referencia o \textit{baseline}.
...
\begin{itemize}
    ...
    \end{quote}
    \textbf{Análisis:} Esta es una sugerencia de alta calidad. No implementar `equals()` y `hashCode()` en DTOs es una fuente común de errores sutiles y difíciles de depurar cuando estos objetos se utilizan en colecciones como `HashMap` o `HashSet`. Este tipo de feedback proactivo ayuda a mejorar la robustez general de la aplicación.
...
\begin{itemize}
    ...
    \item \textbf{Nuevos Criterios de Diferenciación}: Dado que todos los modelos de IA alcanzaron la máxima puntuación en detección, la diferenciación entre ellos ya no se basa en la cobertura, sino en otros factores cualitativos y cuantitativos:
        \begin{itemize}
            \item \textbf{Velocidad y Coste}: Como se observó previamente, existe un claro compromiso. Modelos como \texttt{google/gemini-2.0-flash-exp:free} ofrecen resultados casi instantáneos, mientras que otros como \texttt{deepseek/deepseek-r1-0528:free} son más lentos, lo que puede influir en la elección para distintos tipos de pipelines (CI vs. CD).
            ...
            \item \textbf{Análisis de Calidad de Código}: Además de la seguridad, modelos como \texttt{deepseek/deepseek-r1-0528:free} también proporcionaron un análisis más rico sobre la calidad general del código, similar al de SonarQube, aportando un valor añadido.
        \end{itemize}

    \item \textbf{Complementariedad Re-evaluada}: La idea de complementariedad sigue siendo válida, pero con un nuevo matiz. SonarQube es fundamental para aplicar un conjunto de reglas de "higiene" de código de forma rápida y consistente. La IA, por su parte, no solo complementa, sino que asume el rol principal en la detección de vulnerabilidades de seguridad, ofreciendo una cobertura que antes solo era posible con herramientas comerciales muy costosas o revisiones manuales exhaustivas.
\end{itemize}
...
Mientras que SonarQube se mantiene como una herramienta indispensable para gobernar y mantener la calidad, la fiabilidad y la mantenibilidad del código base, los modelos de IA evaluados han demostrado llenar un vacío crítico en la estrategia de DevSecOps. Su capacidad para realizar un análisis semántico y contextual del código les permite identificar riesgos de seguridad de alto impacto (como RCE o SQLi) que a menudo pasan desapercibidos para los analizadores basados en reglas estáticas.
...
\begin{itemize}
    ...
    \item \textbf{Para Máxima Precisión y Cobertura en Seguridad}: El modelo \texttt{deepseek/deepseek-r1-0528:free} demostró ser la opción más segura y exhaustiva, ideal para ejecuciones en pipelines de despliegue a producción o en escaneos de seguridad programados y profundos donde la cobertura total es el objetivo principal.
    \item \textbf{Para un Balance Óptimo entre Seguridad y Agilidad en CI/CD}: Los modelos \texttt{qwen/qwen-2.5-coder-32b-instruct:free} y \texttt{deepseek/deepseek-chat:free} ofrecen un excelente compromiso, detectando la gran mayoría de las vulnerabilidades críticas en un tiempo de ejecución que se adapta bien a un pipeline de desarrollo activo.
\end{itemize}
...
% FIN DEL CAPÍTULO 4
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Análisis de Resultados de la Ejecución del Pipeline}
\label{subsec:analisis_resultados_pipeline}

Para la validación empírica de la solución, se seleccionó el modelo de IA google/gemini-2.0-flash-exp:free. Esta elección se basa en los resultados obtenidos en la evaluación comparativa de modelos detallada en la sección \ref{subsec:analisis_comparativo_resultados}, el cual evidenció el mejor equilibrio entre precisión, calidad de las sugerencias y eficiencia.
...
\begin{enumerate}
    ...
    \item La etapa \textbf{AI-Powered Analysis} se completó satisfactoriamente, cumpliendo su objetivo principal (\textbf{RF-05}). El script de Python interactuó con la API de la IA y generó los artefactos cruciales: el reporte HTML para el desarrollador (`ai-analysis-report.html`) y el reporte JSON (`analysis-results.json`) para la automatización, cumpliendo así con \textbf{RF-06}.
...
Este fallo controlado es un éxito rotundo para el experimento. Demuestra que el sistema funciona como un mecanismo de seguridad preventivo eficaz (cumpliendo \textbf{RF-07}), deteniendo físicamente la promoción de código inseguro a través del ciclo de vida de desarrollo.
...
El resultado final fue un pipeline completamente exitoso, con todas las etapas marcadas en verde, como se puede observar en la \textbf{Figura \ref{fig:pipeline_exitoso}}. Esto demuestra que la idea principal de la tesis funciona en la práctica: el sistema es capaz de guiar a un desarrollador para transformar un código base inseguro en uno seguro, y de validar automáticamente esta corrección para permitir su despliegue, completando así el ciclo DevSecOps de manera eficiente.
% Usamos [H] del paquete 'float' para forzar la posición exacta de la figura.
\begin{figure}[H] 
    \centering
